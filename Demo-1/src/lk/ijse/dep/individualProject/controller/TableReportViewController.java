...
public class TableReportViewController {

    @FXML
    private TableView<TableReportTM> tblReport;
    ...
    static LocalDate localDate ; // For Data Transfer Purpose
    static int categoryID ;      // For Data Transfer Purpose
    static String categoryName;  // For Data Transfer Purpose
    static String expenseType ;  // For Data Transfer Purpose
    ...
    public void initialize(){
        ...
        tblReport.getColumns().get(0).setCellValueFactory(new PropertyValueFactory<>("categoryName"));
        tblReport.getColumns().get(1).setCellValueFactory(new PropertyValueFactory<>("categoryTotal"));
        tblReport.getColumns().get(2).setCellValueFactory(new PropertyValueFactory<>("percentage"));
    ...
    void clickHome(MouseEvent event){
        ...
        Stage primaryStage = (Stage) tblReport.getScene().getWindow();
        Parent root = null;
        try {
    ...
    private void refresh() {
        ...
        for (TableReportDTO t : categoryTotal) {
            ...
            tblReport.getItems().add(new TableReportTM(t.getCategoryID(),t.getCategoryName(),cTotal,((t.getCategoryTotal()/total)*100)+"%"));
        }
    ...
    public void clickOntheTable(MouseEvent mouseEvent) {
        if(mouseEvent.getClickCount() == 2){

            localDate = datePicker.getValue();
            categoryID = tblReport.getSelectionModel().getSelectedItem().getCategoryID();
            categoryName = tblReport.getSelectionModel().getSelectedItem().getCategoryName();
            if(radioExpense.isSelected()){
                expenseType = "Expense";
            }else if(radioIncome.isSelected()){
                expenseType = "Income";
            ...
            Parent root = null;
            try {
                root = FXMLLoader.load(this.getClass().getResource("/lk/ijse/dep/individualProject/view/DrillDownTableReport.fxml"));
            } catch (IOException e) {
                Logger.getLogger("").log(Level.SEVERE,null,e);
            }
            Scene scene = new Scene(root);
